...
class DetailHistoryViewController: UIViewController {
    ...
    func configTabelView() {
        tabelView.register(UINib(nibName: "DetailHistoryTableViewCell", bundle: nil), forCellReuseIdentifier: "DetailHistoryTableViewCell")
...
extension DetailHistoryViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.setData(name: checkMenu?.menuItem.name ?? "", price: checkMenu?.menuItem.price ?? 0, note: checkMenu?.notes ?? "", quantity: checkMenu?.amout ?? 0)
